...
from RemiRobot import (DEV_USERS, OWNER_ID, DRAGONS, DEMONS, TIGERS, WOLVES,
                          INFOPIC, dispatcher, sw)
from RemiRobot.__main__ import STATS, TOKEN, USER_INFO
import RemiRobot.modules.sql.userinfo_sql as sql
from RemiRobot.modules.disable import DisableAbleCommandHandler
from RemiRobot.modules.sql.global_bans_sql import is_user_gbanned
from RemiRobot.modules.sql.afk_sql import is_afk, check_afk_status
from RemiRobot.modules.sql.users_sql import get_user_num_chats
from RemiRobot.modules.helper_funcs.chat_status import sudo_plus
from RemiRobot.modules.helper_funcs.extraction import extract_user
from RemiRobot import telethn as SaitamaTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThis user is a Developer of Remi!"
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe person here is one of the Shadow Hokage, Their Ranking is UCHIHA GHOST."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThis person here is one of the Assassination Squad, Yuichiro, Remi."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThis user is one of those JONIN, Their Group is called the Deadly Trio."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThis user is one of the Member of Yuichiro, The Three Way DEADLOCK"
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/CrowdXStrike/13">what is rank</a>]'.format(
            bot.username)
